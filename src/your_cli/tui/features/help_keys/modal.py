...
class HelpModal(ModalScreen[None]):
    ...
    def __init__(self, rows: list[tuple[str, str, str]]) -> None:
        super().__init__()
        self._rows = rows  # renamed from _bindings to avoid clash with Textual's internal

    def compose(self) -> ComposeResult:
        with Vertical(id="help-modal-body"):
            yield Label("[b]Keyboard Reference[/b]", id="help-modal-title")
            tbl: DataTable[str] = DataTable(id="help-table", show_cursor=False)
            yield tbl
    ...
    def on_mount(self) -> None:
        ...
        for key, action, desc in self._rows:
            if desc:
