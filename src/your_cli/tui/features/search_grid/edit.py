"""EditJobScreen — label-form editor pushed from SearchGridDemoScreen."""
...
class EditJobScreen(Screen[None]):
        ...
        with ScrollableContainer(id="ej-body"):
            with Vertical(id="ej-form"):
                ...
                with Horizontal(classes="ej-row"):
                    ...
                    yield Select(_TYPE_OPTS, prompt="Select…", id="ej-type",
                                 allow_blank=True)
                ...
                with Horizontal(classes="ej-row"):
                    ...
                    yield Select(_STATUS_OPTS, prompt="Select…", id="ej-status",
                                 allow_blank=True)
                with Horizontal(classes="ej-row"):
                    yield Label("Priority", id="lbl-ej-priority", classes="ej-label")
                    yield Select(_PRIORITY_OPTS, prompt="Select…", id="ej-priority",
                                 allow_blank=True)
                with Horizontal(classes="ej-row"):
                    yield Label("Environment", id="lbl-ej-env", classes="ej-label")
                    yield Select(_ENV_OPTS, prompt="Select…", id="ej-env",
                                 allow_blank=True)

                yield Rule(line_style="dashed")
                yield Static("[dim]Execution[/dim]", classes="ej-section")
                with Horizontal(classes="ej-row"):
                    yield Label("Submitted by", id="lbl-ej-submitted-by",
                                classes="ej-label")
                ...
                with Horizontal(classes="ej-row"):
                    yield Label("Max concurrent jobs  [dim]1–20[/dim]",
                                id="lbl-ej-max-jobs", classes="ej-label")
                ...
                with Horizontal(classes="ej-row"):
                    yield Label("Timeout  [dim](minutes)[/dim]",
                                id="lbl-ej-timeout", classes="ej-label")
    ...
    def on_mount(self) -> None:
        ...
        if not rec:
            return
        self.query_one("#ej-title",        Static).update(
            f"[b]Edit Job[/b]  [dim]{rec['id']}[/dim]")
    ...
    _REQUIRED_SELECTS: frozenset[str] = frozenset({
        "ej-type", "ej-status", "ej-priority", "ej-env",
    })

    def _set_field_error(self, field_id: str, has_error: bool,
                         message: str = "") -> None:
    ...
    def action_save(self) -> None:
        ...
        def _check_select(field_id: str, label: str, value: object) -> None:
            ...
            self._set_field_error(field_id, not ok,
                                  f"{label}: Required" if not ok else "")

        def _check_input(field_id: str, label: str,
        ...
        _check_select("ej-type",        "Type",
                      self.query_one("#ej-type",     Select).value)
        _check_select("ej-status",      "Status",
                      self.query_one("#ej-status",   Select).value)
        _check_select("ej-priority",    "Priority",
                      self.query_one("#ej-priority", Select).value)
        _check_select("ej-env",         "Environment",
                      self.query_one("#ej-env",      Select).value)
        ...
        rec.update({
            ...
            "type":         self.query_one("#ej-type",         Select).value,
            "status":       self.query_one("#ej-status",       Select).value,
            "priority":     self.query_one("#ej-priority",     Select).value,
            "environment":  self.query_one("#ej-env",          Select).value,
