...
class AutocompleteDemoScreen(Screen[None]):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        parts = []
        if tenant:  parts.append(f"tenant=[b]{tenant}[/b]")
        if jobtype: parts.append(f"type=[b]{jobtype}[/b]")
        if user:    parts.append(f"by=[b]{user}[/b]")
        if tag:     parts.append(f"tag=[b]{tag}[/b]")
