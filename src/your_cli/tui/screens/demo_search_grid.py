...
from textual.widgets import (
    Button, DataTable, Footer, Header, Input, Label, Rule, Select, Static, Switch,
)
...
class EditJobScreen(Screen[None]):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        match event.button.id:
            case "btn-ej-save":   self.action_save()
            case "btn-ej-cancel": self.action_go_back()
...
class SearchGridDemoScreen(Screen[None]):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        pages = max(1, (total + self._page_size - 1) // self._page_size)
        match event.button.id:
            case "btn-sg-search": self._run_search()
            case "btn-sg-clear":  self._clear_search()
            case "btn-sg-first":
                if self._page != 0:
                    self._page = 0
                    self._load_page()
